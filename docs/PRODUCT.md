...
Each row opens an exact Active-league Trade search in a new tab for the unique and base type. The locally saved Trade settings cover minimum item level from 65 through 85, corrupted-item inclusion, online status, and listing time. Searches never add a maximum price. Changing the item level also recalculates displayed Dust values and Rankings.
